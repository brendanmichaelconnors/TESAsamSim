...
For now we will set the model to estimate time-varying alpha `alpha_vary=TRUE` but not beta `beta_vary=TRUE`, even thought the simulated data are based on a stationary alpha and beta.
...
# first read in simulate data for scenario with declining productivity
simDatadeclineProd <- read.csv(here("outputs", "simData", "example",
                         "ref_declineProd_CUsrDat.csv"),
                       stringsAsFactors = F)

simDatadeclineProdyCU1 <- simDatadeclineProd %>% 
  filter(CU==1 & iteration ==1)

colnames(simDatadeclineProdyCU1)[c(2,4,5,8,9)] <- c("byr","spwn", "rec", "alpha_true", "beta_true")

# then fit DLM
dlm_model <- fitDLM(data = simDatadeclineProdyCU1,
              alpha_vary = TRUE,
              beta_vary = FALSE)
...
# first read in simulate data for scenario with declining productivity
simDatadeclineCapacity <- read.csv(here("outputs", "simData", "example",
                         "ref_declineCapacity_CUsrDat.csv"),
                       stringsAsFactors = F)

simDatadeclineCapacityCU1 <- simDatadeclineCapacity %>% 
  filter(CU==1 & iteration ==1)

colnames(simDatadeclineCapacityCU1)[c(2,4,5,8,9)] <- c("byr","spwn", "rec", "alpha_true", "beta_true")

# then fit DLM
dlm_model <- fitDLM(data = simDatadeclineCapacityCU1,
              alpha_vary = FALSE,
              beta_vary = TRUE)
...
# first read in simulate data for scenario with declining productivity and capacity
simDatadeclineProdCapacity <- read.csv(here("outputs", "simData", "example",
                         "ref_declineProd&Capacity_CUsrDat.csv"),
                       stringsAsFactors = F)

simDatadeclineProdCapacityCU1 <- simDatadeclineProdCapacity %>% 
  filter(CU==1 & iteration ==1)

colnames(simDatadeclineProdCapacityCU1)[c(2,4,5,8,9)] <- c("byr","spwn", "rec", "alpha_true", "beta_true")

# then fit DLM
dlm_model <- fitDLM(data = simDatadeclineProdCapacityCU1,
              alpha_vary = TRUE,
              beta_vary = TRUE)
...
Looks like the model has a hard time estimating both alpha and beta, for this particular simulated dataset when both cange the same magnitude and direction over the same time period.  
